...
\begin{proposition}
Let $M$ be smooth manifold and let $A : M \to T^{(1,k)}TM$ be a rough section. Then $A \in \Gamma(T^{(1,k)}TM)$ if and only if for all $X_1,\dots,X_k \in \Xfrak(M)$, the rough section $A(X_1,\dots,X_k) : M \to TM$ defined by
\begin{equation}
...
\begin{proof}
Let $(U,(x^i))$ be a smooth chart. Then write $X_1,\dots,X_k \in \Xfrak(M)$ as 
\begin{equation*}
...
\end{equation*} 
\noindent is smooth for $i = 1,\dots,n$. Thus again by the smoothness criterion together with the localness of smoothness \cite[35]{lee:smooth_manifolds:2013} we get that $A(X_1,\dots,X_k) \in \Xfrak(M)$. Conversly, each vector field locally defined by 
\begin{equation*}
X_{j_\nu} = \delta^{\mu_\nu}_{j_\nu} \frac{\partial}{\partial x^{\mu_\nu}}.
\end{equation*}
...
\end{equation*}
\noindent we get that $A^i_{j_1 \dots j_k}$ is smooth and hence $A$.
\end{proof}
